...
public class SpinnerSelectionTest {
    ...
    public void iterateSpinnerItems() {
        String[] spinArray = mActivityRule.getActivity()
                .getResources().getStringArray(R.array.labels_array);
        int size = spinArray.length;

        for (int i=0; i<size; i++) {
            // Click the spinner
            onView( withId(R.id.label_spinner) ).perform( click() );
            // Click specific item in spinner
            onData( is(spinArray[i]) ).perform( click() );
            // Check if the textview contains text from the spinner labels array
            onView( withId(R.id.text_phonelabel) )
                    .check( matches( withText( containsString(spinArray[i]) ) ) );
        }
    }
